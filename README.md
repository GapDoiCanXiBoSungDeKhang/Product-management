...
Phần lớn thời gian tập trung làm quen với luồng cơ bản (CRUD, auth, RBAC, upload, OTP...), nên cách liên kết dữ liệu giữa các collection còn đơn giản, dẫn tới vài chỗ bị N+1 query thay vì gom lại query một lần bằng `$in`:

- `helper/showBlogCreateAndEdit.js` — hàm `showDataIndex()` dùng ở các trang danh sách admin: lặp qua từng bản ghi rồi gọi `Account.findOne()` riêng để lấy tên người tạo/người sửa.
- `helper/showDetailProducts.js` — hàm `detail()` dùng ở trang giỏ hàng và checkout: lặp qua từng sản phẩm trong giỏ rồi gọi `Product.findOne()` riêng cho từng sản phẩm.
...
7. showBlogHelper.showDataIndex()   → enrich với thông tin creator (lặp gọi Account.findOne() từng dòng — xem mục N+1 query ở phần Quá trình xây dựng)
