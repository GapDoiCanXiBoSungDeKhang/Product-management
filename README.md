...
PATCH /admin/products/restore/:id       (khôi phục từ Trash)
    └─→ Product.updateOne({ deleted: false })

DELETE /admin/products/forceDelete/:id  (xóa vĩnh viễn)
...
```
GET /admin/products?status=active&keyword=áo&sort=price-desc&page=2&limit=8
...
4. paginationHelper(...)            → tính skip, totalPage, currentPage
5. filterSort(req.query)            → tạo sort object { price: -1 }
6. Product.find(find).sort().limit().skip()  → lấy data
...
| Method | Path | Mô tả | Permission Required |
|---|---|---|---|
...
| POST | `/admin/products/create` | Tạo sản phẩm | Auth + `products_edit` |
| GET | `/admin/products/edit/:id` | Form sửa sản phẩm | Auth |
| PATCH | `/admin/products/edit/:id` | Cập nhật sản phẩm | Auth + `products_edit` |
| PATCH | `/admin/products/change-status/:id` | Toggle status | Auth + `products_edit` |
| PATCH | `/admin/products/change-multi` | Bulk action | Auth |
| DELETE | `/admin/products/deleteItem/:id` | Soft delete | Auth + `products_delete` |
| GET | `/admin/products/trash` | Thùng rác | Auth |
| PATCH | `/admin/products/restore/:id` | Khôi phục | Auth + `products_delete` |
| DELETE | `/admin/products/forceDelete/:id` | Xóa vĩnh viễn | Auth + `products_delete` |
...
| GET | `/admin/roles` | Nhóm quyền | Auth |
| GET | `/admin/roles/permission` | Phân quyền | Auth |
| PATCH | `/admin/roles/permission` | Cập nhật quyền | Auth + `roles_permissions` |
| GET | `/admin/accounts` | Tài khoản admin | Auth |
| GET | `/admin/users` | Người dùng client | Auth |
| GET | `/admin/checkout` | Đơn hàng | Auth |
...
| GET | `/search?keyword=...` | Tìm kiếm | — |
| GET | `/blogs` | Danh sách blog | — |
| GET | `/blogs/:slug` | Chi tiết blog | — |
| GET | `/cart` | Giỏ hàng | — |
| POST | `/cart/add/:id` | Thêm vào giỏ | — |
| PATCH | `/cart/update/:id` | Cập nhật số lượng | — |
| DELETE | `/cart/delete/:id` | Xóa khỏi giỏ | — |
| GET | `/checkout` | Trang thanh toán | — |
| POST | `/checkout/order` | Đặt hàng | — |
| GET | `/checkout/success/:id` | Đặt hàng thành công | — |
| POST | `/checkout/cancel/:id` | Hủy đơn hàng | Required |
| GET | `/history` | Lịch sử đơn hàng | Required |
...
| GET/POST | `/user/edit` | Chỉnh sửa thông tin | Required |
| GET/POST | `/user/password/forgot` | Quên mật khẩu | — |
| GET/POST | `/user/password/otp` | Nhập OTP | — |
| GET/POST | `/user/password/reset` | Đặt mật khẩu mới | — |
...
> ⚠️ **Lưu ý bảo mật:** File `.env` đã được thêm vào `.gitignore`. Không commit file này lên repository.
...
### Soft Delete
Thay vì xóa thẳng khỏi database, các document được đánh dấu `deleted: true`. Tất cả query bình thường đều filter `{ deleted: false }`, còn trang Trash sẽ query `{ deleted: true }` để hiển thị và cho phép khôi phục hoặc xóa vĩnh viễn.
...
### Audit Trail
Mỗi document lưu lại thông tin ai tạo, ai sửa (mảng — mỗi lần sửa push thêm một entry), ai xóa. Hữu ích khi cần truy vết thay đổi.
...
### Xác thực bằng Cookie Token (không dùng JWT)
Mỗi Account/User có một trường `token` là chuỗi random được tạo sẵn. Khi đăng nhập thành công, server set cookie bằng giá trị token đó. Mỗi request sau đọc cookie → query DB để tìm user tương ứng. Cách này đơn giản nhưng stateful (mỗi request đều cần hit DB).

### Snapshot giá khi đặt hàng
Khi tạo Order, giá và % giảm giá của sản phẩm được copy vào trong order, không reference live sang collection product. Điều này đảm bảo lịch sử đơn hàng không bị ảnh hưởng khi admin thay đổi giá sau đó.
